...
for(i in 1:chosenRNABatchSize) for(j in 1:48){
  wildTypeMu <- sample(parameters$alpha[,1,j],1) * sample(parameters$mu[,1,i],1)
  simulatedWildTypeCounts[i,j] <- rlnorm(1,wildTypeMu,sample(parameters$sigma[,1,i],1,replace=TRUE))
}
simulatedWildTypeCounts

simulatedMutantCounts <- matrix(nrow=chosenRNABatchSize,ncol=48)
for(i in 1:chosenRNABatchSize) for(j in 1:48){
  mutantMu <- sample(parameters$alpha[,2,j],1) * sample(parameters$mu[,2,i],1)
  simulatedMutantCounts[i,j] <- rlnorm(1,mutantMu,sample(parameters$sigma[,2,i],1,replace=TRUE))}
