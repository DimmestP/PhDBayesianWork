...
algos <- list("DESeq"=runDESeq,"DESeq2"=runDESeq2,"edgeR"=runEdgeR,"edgeR-robust"=runEdgeRRobust,
              "DSS"=runDSS,"voom"=runVoom,"SAMseq"=runSAMseq,"EBSeq"=runEBSeq,"Bayesian"=runBayes)
...
algos <- list("DESeq2"=runDESeq2,"Bayesian"=openBayes,"edgeR"=runEdgeR)
...
sampleExperimentalData <- function(m) {
  WTmatrix = as.matrix(WTData[sample(colnames(WTData),m/2)])
  MTmatrix = as.matrix(MTData[sample(colnames(MTData),m/2)])
...
groundBayesTruth = openBayes(ExpressionSet(matrix(nrow=5755,ncol=96),annotation=as.character(1)))

# use 48 rep version of DESeq2 as "ground truth"
groundDEseq2Truth = runDESeq2(ExpressionSet(sampleExperimentalData(96),AnnotatedDataFrame(data.frame(condition = factor(rep(c("A","B"), each = 48))))))

# use 48 rep version of edgeR as "ground truth"
groundEdgeRTruth = runEdgeR(ExpressionSet(sampleExperimentalData(96),AnnotatedDataFrame(data.frame(condition = factor(rep(c("A","B"), each = 48))))))

betaBayes <- ( groundBayesTruth$beta > 0.1 & groundBayesTruth$padj < 0.00001)

betaDEseq2 <- (groundDEseq2Truth$beta > 0.1 & groundDEseq2Truth$padj < 0.00001)

betaEdgeR <- (groundEdgeRTruth$beta > 0.1 & groundEdgeRTruth$padj < 0.00001)
