...
# Bootstrap replicate downsampling to have multiple runs of the same number of replicates
...
for(i in 1:length(chosenNumberReplicates)){
  for(j in 1:4){
    ...
    stanDataCurrent <- prepareData(chosenNumberReplicates[i],WTcolNamesRep[[(((i-1) * 4) + j)]],MTcolNamesRep[[(((i-1) * 4) + j)]])
    ...
    metaData <- currentMetaData %>%  
      bind_rows(tibble(file=fileName,repNo=length(WTcolNamesRep),wildRep=WTcolNamesRep,mutRep=MTcolNamesRep,maxRhat = max(summary(nbinlogStanFit)$summary[,10])))
